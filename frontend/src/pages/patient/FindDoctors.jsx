...
const FindDoctors = () => {
  ...
  const fetchDoctors = async () => {
    try {
      ...
      const uniqueSpecs = ['All', ...new Set(data.doctors.map(doc => doc.specialization))]
    ...
    <div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredDoctors.map((doctor) => (
          <div key={doctor._id} className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition flex flex-col">
            ...
            <div className="p-6 flex-1 flex flex-col">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-semibold ${doctor.availability
            ...
            <div className="p-8 space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                <div className="bg-gray-50 p-4 rounded-xl">
                  ...
                  <p className="text-gray-800 font-medium">{selectedDoctor.experience || 'N/A'}</p>
                </div>
                <div className="bg-gray-50 p-4 rounded-xl">
                  <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-1">Hospital</h3>
                  <p className="text-gray-800 font-medium">{selectedDoctor.hospital || 'N/A'}</p>
                </div>
                <div className="bg-gray-50 p-4 rounded-xl">
                  <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-1">Email</h3>
                  <p className="text-gray-800 font-medium text-sm">{selectedDoctor.email || 'N/A'}</p>
                </div>
                <div className="bg-gray-50 p-4 rounded-xl">
                  <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-1">Phone</h3>
                  <p className="text-gray-800 font-medium">{selectedDoctor.phone || 'N/A'}</p>
                </div>
              </div>

              <div>
                <h3 className="text-lg font-bold text-gray-800 mb-3">Availability Status</h3>
                {selectedDoctor.availability ? (
                  <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-center">
                    <p className="text-green-700 font-semibold">Doctor is available for appointments</p>
                  </div>
                ) : (
                  <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-center">
                    <p className="text-red-700 font-semibold">Doctor is currently unavailable</p>
                  </div>
                )}
              </div>
