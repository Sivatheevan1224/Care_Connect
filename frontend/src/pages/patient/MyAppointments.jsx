  ...
  return (
    <div>
      ...
      {appointments.length === 0 ? (
        ...
        <div className="bg-white rounded-xl shadow-md p-12 text-center">
          <h3 className="text-xl font-semibold text-gray-700 mb-2">No {filterStatus.toLowerCase()} appointments</h3>
          <p className="text-gray-500">You don't have any appointments with this status.</p>
        </div>
      ) : (
                ...
                {filteredAppointments.map((appointment) => (
                  <tr key={appointment._id} className="hover:bg-gray-50 transition">
                    <td className="px-6 py-4">
                      <div className="text-sm font-medium text-teal-600">{appointment.doctor?.name || 'N/A'}</div>
                      <div className="text-xs text-gray-500">{appointment.doctor?.specialization || ''}</div>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-600">
                      {formatDate(appointment.date)}<br />
                      <span className="text-xs font-semibold text-teal-600">{appointment.timeSlot}</span>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-600">{appointment.reason || 'Not specified'}</td>
                    <td className="px-6 py-4">
